...
class CrudRoutesFactory {
  ...
  mergeOptions() {
    ...
    this.options.query = Object.assign(
      {},
      module_1.CrudConfigService.config.query,
      query,
    );
  ...
  overrideRoutes(routesSchema) {
    util_1.getOwnPropNames(this.targetProto).forEach((name) => {
      ...
      const route = routesSchema.find((r) => util_1.isEqual(r.name, override));
      if (override && route && route.enable) {
        const interceptors = reflection_helper_1.R.getInterceptors(
          this.targetProto[name],
        );
        const baseInterceptors = reflection_helper_1.R.getInterceptors(
          this.targetProto[override],
        );
        const baseAction = reflection_helper_1.R.getAction(this.targetProto[override]);
        const baseSwaggerParams = swagger_helper_1.Swagger.getParams(
          this.targetProto[override],
        );
        const baseResponseOk = swagger_helper_1.Swagger.getResponseOk(
          this.targetProto[override],
        );
        ...
        reflection_helper_1.R.setAction(baseAction, this.targetProto[name]);
        swagger_helper_1.Swagger.setOperation(
          override,
          this.modelName,
          this.targetProto[name],
        );
  ...
  overrideParsedBodyDecorator(override, name) {
    ...
    if (withBody && parsedBody) {
      ...
      reflection_helper_1.R.setRouteArgs(
        Object.assign({}, routeArgs, {
          [key]: Object.assign({}, baseBodyArg, { index: parsedBody.index }),
        }),
        this.target,
        name,
      );
      if (util_1.isEqual(override, 'createManyBase')) {
        const paramTypes = reflection_helper_1.R.getRouteArgsTypes(
          this.targetProto,
          name,
        );
        const metatype = paramTypes[parsedBody.index];
        ...
        const toCopy = util_1.isIn(metatype, types) || util_1.isNil(metatype);
        if (toCopy) {
          const baseParamTypes = reflection_helper_1.R.getRouteArgsTypes(
            this.targetProto,
            override,
          );
          const baseMetatype = baseParamTypes[1];
  ...
  getPrimaryParam() {
    return util_1
      .objKeys(this.options.params)
      .find((param) => this.options.params[param].primary);
  }
  setBaseRouteMeta(name) {
    this.setRouteArgs(name);
    this.setRouteArgsTypes(name);
  ...
  setRouteArgs(name) {
    ...
    if (util_1.isIn(name, toValidate)) {
      const group =
        util_1.isEqual(name, 'updateOneBase') || util_1.isEqual(name, 'replaceOneBase')
          ? enums_1.CrudValidationGroups.UPDATE
          : enums_1.CrudValidationGroups.CREATE;
      rest = reflection_helper_1.R.setBodyArg(1, [
        validation_helper_1.Validation.getValidationPipe(this.options, group),
      ]);
    }
    reflection_helper_1.R.setRouteArgs(
      Object.assign({}, reflection_helper_1.R.setParsedRequestArg(0), rest),
      this.target,
      name,
    );
  }
  setRouteArgsTypes(name) {
    if (util_1.isEqual(name, 'createManyBase')) {
      ...
      util_1.isEqual(name, 'createOneBase') ||
      util_1.isEqual(name, 'updateOneBase') ||
      util_1.isEqual(name, 'replaceOneBase')
    ) {
      reflection_helper_1.R.setRouteArgsTypes(
        [Object, this.modelType],
        this.targetProto,
        name,
      );
    } else {
  ...
  setDecorators(name) {
    ...
    reflection_helper_1.R.setDecorators(
      util_1.isArrayFull(decorators) ? decorators : [],
      this.targetProto,
      name,
    );
  }
          ...
          .reduce(
            (a, c) =>
              this.options.params[c].primary
                ? a
                : Object.assign({}, a, { [c]: this.options.params[c] }),
            {},
          )
      : this.options.params;
    const pathParamsMeta = swagger_helper_1.Swagger.createPathParasmMeta(params);
    swagger_helper_1.Swagger.setParams(
  ...
  setSwaggerResponseOk(name) {
    const status =
      util_1.isEqual(name, 'createManyBase') || util_1.isEqual(name, 'createOneBase')
        ? common_1.HttpStatus.CREATED
        : common_1.HttpStatus.OK;
    const isArray =
      util_1.isEqual(name, 'createManyBase') || util_1.isEqual(name, 'getManyBase');
    const metadata = swagger_helper_1.Swagger.getResponseOk(this.targetProto[name]);
    const responseOkMeta = swagger_helper_1.Swagger.createReponseOkMeta(
      status,
      isArray,
      this.modelType,
    );
    swagger_helper_1.Swagger.setResponseOk(
      Object.assign({}, metadata, responseOkMeta),
      this.targetProto[name],
    );
  }
}
