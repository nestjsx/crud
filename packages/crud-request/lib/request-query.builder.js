...
class RequestQueryBuilder {
  ...
  static setOptions(options) {
    RequestQueryBuilder._options = Object.assign(
      {},
      RequestQueryBuilder._options,
      options,
      {
        paramNamesMap: Object.assign(
          {},
  ...
  getCondition(cond) {
    ...
    return (
      this[`_${cond}`]
        .map(
          (f) =>
            `${param}=${f.field}${d}${f.operator}${
              util_1.hasValue(f.value) ? d + f.value : ''
            }`,
        )
        .join('&') + '&'
    );
    ...
    return (
      this._join
        .map(
          (j) =>
            `${param}=${j.field}${
              util_1.isArrayFull(j.select) ? d + j.select.join(ds) : ''
            }`,
        )
        .join('&') + '&'
    );
