...
import { ClassType, hasLength, isArrayFull, isNil, isNull, isObject, isUndefined, objKeys } from '../utils';
...
export class TypeOrmCrudService<T> extends CrudService<T> {
  ...
  public async getMany(req: CrudRequest): Promise<GetManyDefaultResponse<T> | T[]> {
    ...
    const builder = await this.createBuilder(parsed, options, true, options.query?.allowGetSoftDeleted);
  ...
  public async getOne(req: CrudRequest): Promise<T> {
    return this.getOneOrFail(req, false, req.options.query?.allowGetSoftDeleted);
  }
