...
describe('#crud-typeorm', () => {
  describe('#query params', () => {
      ...
      it('should return with filter and or, 1', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(res.body.length).toBe(5);
            res.body.forEach((c) => {
              expect(!['Name2', 'Name3'].includes(c.name) || c.domain.includes('5')).toBe(
                true,
              );
            });
            done();
      ...
      it('should return with filter and or, 2', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            res.body.forEach((p) => {
              expect(
                p.name.endsWith('foo') || (p.name.startsWith('P') && p.isActive),
              ).toBe(true);
            });
            done();
          });
      ...
      it('should return with filter and or, 3', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            res.body.forEach((p) => {
              expect(p.companyId > 22 || (p.companyId >= 6 && p.companyId < 10)).toBe(
                true,
              );
            });
            done();
      ...
      it('should return with filter and or, 4', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            res.body.forEach((p) => {
              expect(
                ([6, 10].includes(p.companyId) && p.companyId <= 10) ||
                  (p.isActive === false && !isNil(p.description)),
              ).toBe(true);
            });
            done();
          });
      ...
      it('should return with filter and or, 6', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(res.body.length).toBe(10);
            res.body.forEach((p) => {
              expect(p.companyId >= 1 && p.companyId <= 5).toBe(true);
            });
            done();
          });
      });
      ...
      it('should return joined entity, 1', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(Object.keys(company)).toBeArrayOfSize(2);
            expect(company).toContainKeys(['id', 'name']);
            done();
          });
      });
      it('should return joined entity, 2', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(company.users.length).not.toBe(0);

            company.users.forEach((u) => {
              expect(Object.keys(u)).toBeArrayOfSize(1);
              expect(u).toContainKeys(['companyId']);
            });
            done();
          });
      });
    ...
    describe('#query nested join', () => {
      ...
      it('should return joined entity, 1', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(user.id).toBe(1);
            expect(user.company.name).not.toMatch('invalid');
            done();
          });
      });
      it('should return joined entity, 2', (done) => {
      ...
      it.skip('should return joined entity with alias', (done) => {
        const query = qb
      ...
      it('should return joined entity with ManyToMany pivot table', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            expect(project.company.id).toBe(1);
            expect(project.users).toBeDefined();
            expect(project.users.length).toBe(2);
            expect(project.users[0].id).toBe(1);
            expect(project.users[1].id).toBe(2);
            expect(project.userProjects).toBeDefined();
            expect(project.userProjects.length).toBe(2);
            expect(project.userProjects[0].review).toBe('User project 1 1');
            expect(project.userProjects[1].review).toBe('User project 1 2');
            done();
          });
      });
    });
      ...
      it('should return joined relation', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            ...
            const user = res.body;
            expect(user.id).toBe(1);
            expect(user.userLicenses).toBeDefined();
            expect(user.userLicenses).toBeArrayOfSize(3);
            user.userLicenses.forEach((ul) => {
              expect(ul.userId).toBe(1);
            });
            expect(user.userLicenses.map((ul) => ul.licenseId)).toContainValues([
              1,
              2,
              4,
            ]);
      ...
      it('should throw 400 if SQL injection has been detected', (done) => {
        ...
        return request(server)
          ...
          .end((_, res) => {
            expect(res.status).toBe(400);
            expect(res.body.message).toBe(
              `Invalid column name ' ASC; SELECT CAST( version() AS INTEGER); --'`,
            );
            done();
          });
      });
    ...
    describe('#search', () => {
      ...
      it('should return with search, 8', async () => {
        const query = qb
          .search({ id: { $notnull: true, $or: { $eq: 1, $in: [3, 4] } } })
          .query();
        const res = await projects2()
          .query(query)
          .expect(200);
        expect(res.body).toBeArrayOfSize(3);
        expect(res.body.map((p) => p.id)).toContainValues([1, 3, 4]);
      });
      ...
      it('should return with search, 11', async () => {
        ...
        expect(res.body.map((p) => p.id)).toContainValues([1, 2, 3, 4]);
        res.body.forEach((p) => {
      ...
      it('should return with search, 12', async () => {
        ...
        expect(res.body.map((p) => p.id)).not.toContainValues([5, 6, 7, 8, 9, 10]);
      });
      ...
      it('should return with search, 16', async () => {
        ...
        expect(res.body[0].name).toBe('Project3');
      });
      ...
      it('should return with default filter, 1', async () => {
        ...
        expect(res.body[0].name).toBe('Project11');
      });
      ...
      it('should return with $eqL search operator', async () => {
        ...
        const res = await projects4()
          ...
          .expect(200);
        expect(res.body).toBeArrayOfSize(1);
        expect(res.body[0].name).toEqualCaseInsensitive('project1');
      });
      it('should return with $neL search operator', async () => {
        ...
        res.body.forEach((p) => {
          expect(p.name).not.toEqualCaseInsensitive('project1');
        });
      });
      ...
      it('should return with $contL search operator', async () => {
        ...
        expect(res.body).toBeArrayOfSize(3);
        res.body.forEach((u) => {
          expect(u.email.includes('1@')).toBe(true);
        });
      });
      it('should return with $exclL search operator', async () => {
        ...
        res.body.forEach((u) => {
          expect(u.email.includes('1@')).toBe(false);
        });
      });
      it('should return with $inL search operator', async () => {
        ...
        expect(res.body).toBeArrayOfSize(2);
        res.body.forEach((c) => {
          expect(c.name).toBeOneOf(['Name2', 'Name3']);
        });
      });
      it('should return with $notinL search operator', async () => {
        ...
        res.body.forEach((c) => {
          expect(c.name).not.toBeOneOf(['Project7', 'Project8', 'Project9']);
        });
      });
